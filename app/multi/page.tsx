...
type Issue = { severity: string; quote: string; note: string };
...
function issueCount(analysis: Analysis) {
  return ["linguistic_quality", "terminology_consistency", "cultural_adaptation", "completeness"]
    .reduce((n, k) => n + ((analysis[k as keyof Analysis] as Dimension).issues?.length ?? 0), 0);
}
    ...
    <main style={{ maxWidth: 820, margin: "40px auto", padding: "0 20px", fontFamily: "sans-serif" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14, marginBottom: 24 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <button
            onClick={handleRun}
            disabled={running || !selected.length || !template.includes("{lang}")}
            style={{
